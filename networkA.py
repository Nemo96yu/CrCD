...
class networkA(nn.Module):
    ...
    def forward(self, img_q, key=None, partial_Y=None, args=None, eval_only=False):
        ...
        for feat_q, label in zip(q, pseudo_labels_b):
            self.prototypes[label] = self.prototypes[label] * args.proto_m + (1 - args.proto_m) * feat_q
        # 归一化原型
        self.prototypes = F.normalize(self.prototypes, p=2, dim=1)

        features = torch.cat((q, key, self.queue.clone().detach()), dim=0)
        pseudo_labels = torch.cat((pseudo_labels_b, pseudo_labels_b, self.queue_pseudo.clone().detach()), dim=0)
        labels = logits_prot
        self._dequeue_and_enqueue(key, pseudo_labels_b, args)
        W = 0
        return output, features, score_prot, labels, prototypes, W
